...
---
Building
Ensure Theos is installed and the `THEOS` environment variable is set in your shell
Clone the repo and `cd` into it
Run `make clean && make` — the compiled dylib will be at `.theos/obj/SpotifyATVAdBlock.dylib`
Injecting
Extract the IPA with `unzip Spotify.ipa -d SpotifyPatched`
Copy the dylib into the app bundle at `SpotifyPatched/Payload/TvOSApp.app/`
Run `insert_dylib --strip-codesig --all-yes @executable_path/SpotifyATVAdBlock.dylib SpotifyPatched/Payload/TvOSApp.app/TvOSApp` to inject the load command
Repack the folder back into an IPA with `zip -qr SpotifyATVAdBlock_patched.ipa Payload/` from inside `SpotifyPatched/`
Signing & Installing
Sign the patched IPA using your resign script, passing your Apple Development certificate identity and provisioning profile
Find your Apple TV's UDID in Xcode under Window → Devices and Simulators, or run `xcrun devicectl list devices`
Install with `xcrun devicectl device install app --device <UDID> SpotifyATVAdBlock_signed.ipa`
---
